...
router.get("/public/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deck = await Deck.findOne({ _id: id, isPublic: true })
      .populate("user", "username")
      .populate("cards.cardId");
    if (!deck) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const decks = await Deck.find({ user: req.user.id }).populate(
      "cards.cardId",
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (format === "Commander" && commander) {
      const cardRecord = await Card.findOneAndUpdate(
        { name: commander, user: req.user.id },
      ...
      if (cardRecord) {
        initialCards.push({
          cardId: cardRecord._id,
          quantity: 1,
          color_identity: cardRecord.color_identity,
        });
      }
    }
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const deck = await Deck.findOne({ _id: id, user: req.user.id }).populate(
      "cards.cardId",
    );
    if (!deck) {
      return res
        .status(404)
        .json({ error: `Deck not found or does not belong to user` });
    }

    const deckObj = deck.toObject();
    if (deckObj.format === "Commander" && deckObj.commander) {
      const commanderDetails = await Card.findOne({
        name: deckObj.commander,
        user: req.user.id,
      });
...
router.delete("/:id/remove-card-instance", async (req, res) => {
  ...
  if (!entryId) {
    return res
      .status(400)
      .json({ error: "cardId is required in request body" });
  }
  try {
    let deck;
    deck = await Deck.findOneAndUpdate(
    ...
    return res.json({
      deck: deck,
      message: "Card instance removed successfully",
    });
  } catch (err) {
...
router.post("/sync-card", async (req, res) => {
  ...
  try {
    ...
    const card = await Card.findOneAndUpdate(
      { scryfallId: scryFallCardData.id, user: userId },

      {
        ...
        oracle_text: scryFallCardData.oracle_text,
        artist:
          scryFallCardData.artist ||
          (scryFallCardData.card_faces
            ? scryFallCardData.card_faces[0].artist
            : "Unknown Artist"),
        scryfallId: scryFallCardData.id,
        image_uris: scryFallCardData.image_uris,
        legalities: scryFallCardData.legalities,
...
router.get("/admin/repair-cards", async (req, res) => {
  try {
    ...
    return res.json({
      message: `Updated ${updatedCount} cards with missing CMC`,
    });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
