...
const rollingDie = (sides) => {
  console.log(
    `rolled a ${sides} -sided die and got ${Math.floor(Math.random() * sides) + 1}`,
  );
};
  ...
  return (
    <div className="dice dice_container">
      <div className="dice_buttons">
        ...
        <button
          className=" buttons dice_button dice_button-roll"
          onClick={rollDice}
        >
          {" "}
          roll the dice{" "}
        ...
        <button
          className="buttons dice_button dice_button-reset"
          onClick={resetDice}
        >
          Reset Dice
        </button>
        <span className="dice_span dice_span-number">
          number of dice {groupOfDice.length}
        </span>
        <span className="dice_span dice_span-total">{total}</span>
        {dieResults.map((result, index) => (
          <span className="dice_span dice_span-die" key={index}>
            {" "}
            Die {index + 1}: {result}{" "}
          </span>
        ))}

        <Link className="links dice_link" to="/dashboard">
          {" "}
          Back to Dashboard{" "}
        </Link>
      </div>
