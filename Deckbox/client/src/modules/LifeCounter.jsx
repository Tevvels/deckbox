...
function LifeCounter() {
  ...
  const updateLifeTotal = (id, newValue) => {
    setLifeTotal((prevLife) =>
      prevLife.map((life) =>
        life.id === id ? { ...life, value: newValue } : life,
      ),
    );
  };
  ...
  const renderPlayers = () => {
    ...
    for (let i = 0; i < playerCount && playerCount <= 6; i++) {
      players.push(
        <>
          <div className="life_counter-player" key={i}>
            Player {i + 1}
          </div>
          <button
            className="buttons life_counter-player-setLife"
            onClick={() => setLifeTotal((lifeTotal) => lifeTotal + 1)}
          >
            {lifeTotal}
          </button>
        </>,
  ...
  return (
    <div className="life_counter life_counter-container">
      ...
      <input
        className="inputs life_counter-player-count"
        type="number"
        value={handleInput}
        onChange={inputValueChange}
        placeholder="What is the number of players?"
      />
      <button
        className="buttons submit life_counter-player-countsub"
